...
const doesRoomHaveStorage = (roomNumber: number): boolean => {
  ...
  if ((roomNumber >= 15 && roomNumber <= 22) || 
...
export default function Home() {
  ...
  const [currentWorkingDayString, setCurrentWorkingDayString] = useState<string>(""); // State to store which day's schedule is shown
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const scheduleResponse = await fetch('http://localhost:8000/api/schedule');
          if (scheduleResponse.ok) {
            const scheduleData = await scheduleResponse.json();
            if (scheduleData && Object.keys(scheduleData).length > 0) {
              schedule = scheduleData;
              console.log("Successfully loaded schedule from API:", 
                          Object.keys(schedule).length, "dates available");
            } else {
              console.warn("Schedule from API is empty, using hardcoded schedule");
            }
          } else {
            console.warn("Failed to fetch schedule from API:", scheduleResponse.status);
          }
        } catch (fetchError: unknown) {
          console.warn("Error fetching schedule from API:", fetchError instanceof Error ? fetchError.message : String(fetchError));
        ...
        const now = new Date();
        const fivePM = set(now, { hours: 17, minutes: 0, seconds: 0, milliseconds: 0 });
        ...
        console.log("Found working people for", targetDateStr, ":", workingFullNames);
        ...
        if (workingFullNames.length === 0) {
          console.log("No workers found for today, using test data from April 7th");
          ...
          if (workingFullNames07April.length > 0) {
            ...
            const workingShortNames = new Set(
              workingFullNames07April.map(fullName => nameMap[fullName]).filter(Boolean)
            );
            
            const synchronizedEmployees = baseEmployees.map((emp: Employee) => ({
              ...emp,
        ...
        setEmployees(baseEmployees.map((emp: Employee) => ({
          ...emp,
  ...
  const handleGenerateAssignments = async () => {
    ...
    try {
      ...
      if (workingEmps.length === 0) {
        ...
        alert(`Нет работающих сотрудников для распределения задач на ${currentWorkingDayString}.`);
        setLoading(false);
        return;
      }
      ...
      const result: AssignmentResult = {
        ...
        too_many_trolleys: currentAssignments.filter(a => a.tasks.some(task => !doesRoomHaveStorage(task.room))).length > 2
      };
        ...
        currentAssignments.forEach(assignment => {
          ...
          if (!assignment.needs_trolley) {
            ...
            if (needsRecalculation) {
              console.log(`Добавлено ${additionalTimeTotal} минут к заданиям для ${assignment.employee} (без тележки)`);
  ...
  const workingEmployeesCount = employees.filter(emp => emp.istodayworking).length;
  ...
  const resetCorridorTasks = (corridorId: number) => {
    if (confirm(`Вы уверены, что хотите сбросить все типы заданий в коридоре ${corridorId}?`)) {
      setRoomTasks(prevTasks => 
        prevTasks.map(task => 
  ...
  return (
    <section className="flex flex-col gap-2 pt-0 pb-4">
      ...
      <div className="mt-4">
        ...
        <h2 className={title({ size: "sm", class: "text-primary-700 text-base" })}>
          Team Members ({workingEmployeesCount} Working {currentWorkingDayString ? `on ${currentWorkingDayString}` : 'Today'})
        </h2>
              ...
              {roomTasks
                ...
                .reduce((acc: JSX.Element[], task, index, array) => {
                  ...
                  if (isNewCorridor && currentCorridor) {
                    ...
                    const sideText = corridorDetails[currentCorridor]?.side === 'east' ? 'Восточная' : 'Западная';
                        ...
                        <td colSpan={4} className={`py-2 px-3 text-xs font-bold ${textColorClass} ${bgColorClass}`}>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              ...
                              Коридор {currentCorridor}: {sideText} сторона, {corridorDetails[currentCorridor]?.floor || ''} этаж
                              {hasStorage && (
                                ...
                                {editingCorridor === currentCorridor ? (
                                  <>
                                    <div className="flex items-center gap-1">
                                      <span className="text-[10px] text-primary-600">Установить тип:</span>
                                      <Select 
                                        size="sm"
                                        variant="flat"
                                        ...
                                        onSelectionChange={(keys) => {
                                          if (keys instanceof Set) {
                                            ...
                                            setTypeForCorridor(currentCorridor, selectedKey);
                                          }
                                        }}
                                      >
                                        {taskTypes.map((type) => (
                                          <SelectItem 
                                            key={type.value}
                                            className={
                                              type.value === 'V' ? 'text-blue-800 bg-blue-50/60 hover:bg-blue-100 text-xs' :
                                              type.value === 'B' ? 'text-green-800 bg-green-50/60 hover:bg-green-100 text-xs' :
                                              type.value === 'N' ? 'text-orange-800 bg-orange-50/60 hover:bg-orange-100 text-xs' :
                                              type.value === 'V/N' ? 'text-purple-800 bg-purple-50/60 hover:bg-purple-100 text-xs' :
                                              type.value === '-' ? 'text-gray-800 bg-gray-50/60 hover:bg-gray-100 text-xs' :
                                    ...
                                    <Button 
                                      ...
                                      onPress={() => resetCorridorTasks(currentCorridor)}
                                    >
                                      Сбросить все
                                    </Button>
                                    <Button 
                                  ...
                                  <Button 
                                    ...
                                    variant="flat" 
                                    className="text-[10px] h-6 px-1 min-w-0" 
                                    onPress={() => setEditingCorridor(currentCorridor)}
                                  >
              ...
              {[...assignments.assignments] // Клонируем массив перед сортировкой
                    ...
                    <div className="flex flex-col w-full">
                      <div className="flex items-center w-full mb-1">
                        ...
                        <div className="mr-2 flex-shrink-0">
                          {assignment.team_size && assignment.team_size > 1 && assignment.team_members ? (
                            <div className="flex -space-x-2">
                              {assignment.team_members.slice(0, 2).map((member, idx) => (
                                <div key={`avatar-${member}-${idx}`} className="relative w-8 h-8 rounded-full overflow-hidden border border-white">
                                  <EmployeeAvatar name={member} size="md" className="!w-full !h-full" />
                                </div>
                              ))}
                            ...
                            <EmployeeAvatar name={assignment.employee} size="md" className="!w-9 !h-9" />
                          )}
                        </div>
                        ...
                        <div className="flex-grow">
                          <div className="flex items-center">
                            <span className="font-medium text-primary-800 mr-2" title={assignment.employee}>
                            ...
                            <div className="flex items-center space-x-1 flex-shrink-0">
                              ...
                              {assignment.is_overtime && (
                                <Badge color="danger" variant="flat" size="sm" className="text-[10px] font-medium">OT</Badge>
                              )}
                              {/* Индикатор тележки */}
                              {assignment.needs_trolley && (
                                <span title="Нужна тележка" className="px-1 py-0.5 bg-blue-100 text-blue-800 rounded-full text-[10px] font-medium flex items-center">
                          ...
                          <div className="flex items-center justify-between text-[10px] text-primary-600 mt-0.5">
                            <div className="flex items-center space-x-2">
                              <span>Задач: <b>{assignment.tasks.length}</b></span>
                              <span>Время: <b>{assignment.total_effective_time.toFixed()} мин</b></span>
                              <span title={`Ожидаемое время окончания: ${assignment.estimated_finish_time_str}`}>
                                <b className={assignment.is_overtime ? 'text-red-600' : ''}>
                        ...
                        {(() => {
                          ...
                          assignment.tasks.forEach(task => {
                            ...
                            if (originalTask?.type) {
                              stats[originalTask.type as keyof typeof stats] += 1;
                            }
                          });
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        ...
                        {(() => {
                          ...
                          assignment.tasks.forEach(task => {
                            const originalTask = roomTasks.find(rt => rt.room === task.room);
                            if (originalTask?.type) {
                              roomTypeCount[originalTask.type] = (roomTypeCount[originalTask.type] || 0) + 1;
                            }
                          });
                          
                          // Возвращаем чипсы со статистикой по типам комнат
                          return Object.entries(roomTypeCount).map(([type, count]) => {
                            ...
                            const chipStyle = (() => {
                              switch(type) {
                                ...
                                case 'V/N': return 'bg-purple-100 text-purple-800';
                        ...
                        {(() => {
                          ...
                          const noStorageRooms = assignedRooms.filter(room => !doesRoomHaveStorage(room));
                          
                          const noStorageRoomCount = noStorageRooms.length;
                          const roomWord = noStorageRoomCount === 1 ? 'комната' : (noStorageRoomCount >= 2 && noStorageRoomCount <= 4) ? 'комнаты' : 'комнат';
                          
                          return (
                            <span className="flex items-center bg-yellow-100 text-yellow-800 font-medium rounded-full px-2 py-0.5 text-[10px]">
                              {assignment.needs_trolley && (
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-0.5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                  <circle cx="9" cy="21" r="1" />
                                  <circle cx="20" cy="21" r="1" />
                                  <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6" />
                                </svg>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1 mb-0.5">
                        {(() => {
                          ...
                          const corridorsWithDuplicates = assignment.tasks
                            ...
                            .filter((corridor): corridor is number => corridor !== undefined);
                          ...
                          return corridors.map(corridor => {
                            ...
                            const chipColor = (() => {
                              switch(corridor) {
                                ...
                                case 6: return "bg-rose-100 text-rose-800";
                                default: return "bg-gray-100 text-gray-800";
                              }
                            })();
                            ...
                            return (
                              ...
                              >
                                {roomsText}: {rooms.length} {rooms.length === 1 ? 'комната' : rooms.length > 1 && rooms.length < 5 ? 'комнаты' : 'комнат'}
                              </span>
                            );
                          });
                        })()}
                ...
                >
                  ...
                  <div className="border-t border-primary-100 p-3 bg-primary-50/50">
                    ...
                    {assignment.team_size && assignment.team_size > 1 && assignment.team_members && (
                      <div className="mb-3">
                        <h4 className="text-[11px] font-medium text-green-700 mb-1.5">Состав команды:</h4>
                        <div className="flex flex-wrap gap-1">
                          {assignment.team_members.map((member) => (
                            <div key={`team-member-${member}`} className="flex items-center bg-white px-2 py-1 rounded-full border border-green-100">
                              <div className="flex-shrink-0">
                                <EmployeeAvatar name={member} size="md" />
                              </div>
                              <span className="text-xs text-primary-800 ml-1">{member}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="mb-3">
                       <h4 className="text-[11px] font-medium text-primary-700 mb-1.5">Расчет времени:</h4>
                       <div className="space-y-1 text-xs">
                           <div className="flex justify-between"><span>Время задач:</span> <span>{assignment.total_task_time.toFixed(1)} мин.</span></div>
                           ...
                           <div className="flex justify-between">
                             <span>
                               {(() => {
                                 ...
                                 const hasRoomsWithoutStorage = !assignment.needs_trolley && 
                                   assignment.tasks.some(task => !doesRoomHaveStorage(task.room));
                             ...
                             <span>
                               ...
                               {(() => {
                                 ...
                                 if (!assignment.needs_trolley) {
                                   const roomsWithoutStorage = assignment.tasks.filter(
                                     task => !doesRoomHaveStorage(task.room)
                                   ).length;
                                   
                                   if (roomsWithoutStorage > 0) {
                                     const additionalTime = roomsWithoutStorage * 3;
                           ...
                           {assignment.team_size && assignment.team_size > 1 && (
                              <div className="flex justify-between text-green-600 font-medium">
                                ...
                                <span>×{assignment.efficiency_factor?.toFixed(1)}</span>
                              </div>
                            )}
                           {/* Показываем перерыв только если он был добавлен */}
                           {assignment.break_time_added > 0 && (
                              <div className="flex justify-between text-blue-600"><span>Перерыв (10:45-11:00):</span> <span>+ {assignment.break_time_added} мин.</span></div> 
                           )}
                           {/* Итоговое время с указанием овертайма */}
                           <div className={`flex justify-between font-semibold border-t border-dashed border-primary-200 pt-1 mt-1 ${assignment.is_overtime ? 'text-red-600' : 'text-primary-800'}`}>
                               <span>Итоговое время:</span> 
                               <span>{assignment.total_effective_time.toFixed(1)} мин.</span>
                           </div>
                           {/* Расчетное время окончания */}
                            <div className={`flex justify-between text-xs ${assignment.is_overtime ? 'text-red-600 font-semibold' : 'text-gray-600'}`}>
                    ...
                    <h4 className="text-[11px] font-medium text-primary-700 mb-1.5 mt-3 border-t border-primary-100 pt-2">Детали заданий ({assignment.tasks.length}):</h4>
                    {/* Контейнер с прокруткой для списка задач */}
                    <div className="space-y-1.5 max-h-60 overflow-y-auto pr-1">
                      {/* Группируем задания по коридорам для вывода */}
                      {(() => {
                        const tasksByCorridors: Record<string, AssignedTask[]> = {};
                        // Группируем задачи по ключу коридора
                        assignment.tasks.forEach(task => {
                          const corridorKey = task.corridor ? `Коридор ${task.corridor}` : 'Прочее';
                        ...
                        const sortedCorridorKeys = Object.keys(tasksByCorridors).sort((a, b) => {
                        ...
                        return sortedCorridorKeys.map((corridorKey) => (
                          <div key={corridorKey} className="mb-1">
                            ...
                            <div className="text-[10px] font-semibold text-primary-600 mb-1 px-1 sticky top-0 bg-primary-100/80 backdrop-blur-sm py-0.5 rounded-sm z-10">
                              {corridorKey} ({tasksByCorridors[corridorKey].length} задач)
                            </div>
                            ...
                            {tasksByCorridors[corridorKey].map((task, i) => {
                              ...
                              const taskForColor: RoomTask = originalTask 
                                ? originalTask 
                                : { room: task.room, corridor: task.corridor, type: '', note: '' }; 
                              
                              return (
                              <div key={`${task.room}-${i}`} className="flex items-center justify-between bg-white p-1.5 rounded border border-primary-100 text-xs ml-1 hover:border-primary-300">
                                {/* Левая часть: номер комнаты и описание */}
                                <div className="flex items-center overflow-hidden mr-2">
                                  {/* Номер комнаты */}
                                  <span className={`font-medium w-6 text-center flex-shrink-0 ${getRowColor(taskForColor).replace('bg-','text-').replace('-50','-800')}`} title={`Room ${task.room}`}>
                                    {task.room}
                                  </span>
                                  {/* Тип задачи */}
                                  {originalTask?.type && (
                                    <span className={`mx-1 flex-shrink-0 px-1.5 py-0.5 rounded-sm text-[10px] font-medium ${
                                      originalTask.type === 'V' ? 'bg-blue-100 text-blue-800' : 
                                      originalTask.type === 'B' ? 'bg-green-100 text-green-800' : 
                                      originalTask.type === 'N' ? 'bg-orange-100 text-orange-800' : 
                                      originalTask.type === 'V/N' ? 'bg-purple-100 text-purple-800' : 
                                      'bg-gray-100 text-gray-800'
                                    }`} title={getTaskTypeName(originalTask.type)}>
                                      {originalTask.type}
                                    </span>
                                  )}
                                  {/* Описание задачи с title для полного текста */}
                                  <span className="text-primary-700 ml-1.5 truncate" title={task.description}>{task.description}</span>
                                </div>
                                {/* Правая часть: время задачи */}
                                <span className="font-medium text-primary-800 ml-1 flex-shrink-0" title={task.raw_estimated_time ? `Изначально: ${task.raw_estimated_time} мин.` : ''}>
                                  {task.estimated_time.toFixed(1)} мин.
                                  {task.raw_estimated_time && task.raw_estimated_time !== task.estimated_time && (
              ...
              <div className="mt-3 p-2 bg-yellow-100 border border-yellow-200 rounded-lg text-xs text-yellow-800">
                {assignments.too_many_trolleys && (
                  <p className="font-medium mb-1 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1 flex-shrink-0 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <circle cx="9" cy="21" r="1" />
                      <circle cx="20" cy="21" r="1" />
                      <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6" />
                    </svg>
                    <span>Внимание: Требуется более 2 тележек. Назначены сотрудникам с наибольшим количеством комнат без склада. Остальным добавлено время (+3 мин/комната).</span>
                  </p>
                )}
